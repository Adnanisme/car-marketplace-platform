...
<div class="container mx-auto py-4 px-4 sm:py-8">
    <div class="max-w-4xl mx-auto">
        ...
        <form action="{{ route('admin.cars.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6 bg-white p-4 sm:p-6 rounded-lg shadow-lg">
        @csrf
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                <label for="name" class="block text-sm font-semibold text-gray-700 mb-2">Car Name</label>
                <input id="name" name="name" type="text" class="karsource-input" placeholder="Enter car name" required value="{{ old('name') }}" />
            ...
            <div>
                ...
                <input id="mileage" name="mileage" type="text" class="karsource-input number-input" placeholder="e.g., 45,000" required value="{{ old('mileage') }}" />
                <x-input-error :messages="$errors->get('mileage')" class="mt-2" />
            </div>
            <div>
                <label for="year" class="block text-sm font-semibold text-gray-700 mb-2">Year</label>
                <input id="year" name="year" type="number" class="karsource-input" placeholder="e.g., 2020" min="1990" max="{{ date('Y') + 1 }}" required value="{{ old('year') }}" />
            ...
            <div>
                ...
                <select id="transmission" name="transmission" class="karsource-select" required>
                    ...
                    <option value="Automatic Transmission" {{ old('transmission') == 'Automatic Transmission' ? 'selected' : '' }}>Automatic</option>
                    <option value="Manual Transmission" {{ old('transmission') == 'Manual Transmission' ? 'selected' : '' }}>Manual</option>
                </select>
            ...
            <div>
                ...
                <input id="color" name="color" type="text" class="karsource-input" placeholder="e.g., Red, Blue, White" required value="{{ old('color') }}" />
                <x-input-error :messages="$errors->get('color')" class="mt-2" />
            </div>
            <div>
                <label for="engine_size" class="block text-sm font-semibold text-gray-700 mb-2">Engine Size (L)</label>
                <input id="engine_size" name="engine_size" type="number" step="0.1" class="karsource-input" placeholder="e.g., 2.0" required value="{{ old('engine_size') }}" />
            ...
            <div>
                ...
                <input id="horsepower" name="horsepower" type="number" class="karsource-input" placeholder="e.g., 200" required value="{{ old('horsepower') }}" />
                <x-input-error :messages="$errors->get('horsepower')" class="mt-2" />
            </div>
            <div>
                <label for="location" class="block text-sm font-semibold text-gray-700 mb-2">Location</label>
                <input id="location" name="location" type="text" class="karsource-input" placeholder="e.g., Lagos, Abuja" required value="{{ old('location') }}" />
                <x-input-error :messages="$errors->get('location')" class="mt-2" />
            </div>
            <div>
                <label for="registered_city" class="block text-sm font-semibold text-gray-700 mb-2">Registered City</label>
                <input id="registered_city" name="registered_city" type="text" class="karsource-input" placeholder="e.g., Lagos" required value="{{ old('registered_city') }}" />
                <x-input-error :messages="$errors->get('registered_city')" class="mt-2" />
            </div>
            <div>
                <label for="price" class="block text-sm font-semibold text-gray-700 mb-2">Price (₦)</label>
                <input id="price" name="price" type="text" class="karsource-input number-input" placeholder="e.g., 5,500,000" required value="{{ old('price') }}" />
                <x-input-error :messages="$errors->get('price')" class="mt-2" />
            </div>
        </div>
        <div>
            <label for="additional_info" class="block text-sm font-semibold text-gray-700 mb-2">Additional Information</label>
            <textarea id="additional_info" name="additional_info" class="karsource-textarea" rows="4" placeholder="Any additional details about the car...">{{ old('additional_info') }}</textarea>
            <x-input-error :messages="$errors->get('additional_info')" class="mt-2" />
        </div>
        <!-- Visual Upload Interface -->
        <div>
            <label class="block text-sm font-semibold text-gray-700 mb-3">Car Images</label>
            
            <!-- Upload Area -->
            <div id="uploadArea" class="karsource-file-upload cursor-pointer" onclick="document.getElementById('images').click()">
                <input id="images" name="images[]" type="file" class="hidden" accept="image/*" multiple required />
                <div id="uploadHelper" class="file-upload-helper text-center">
                    <i class="fas fa-cloud-upload-alt text-4xl text-blue-600 mb-3"></i>
                    <p class="text-gray-700 font-medium text-lg">Upload Car Images</p>
                    <p class="text-sm text-gray-500 mb-2">Drag and drop or click to select 4-5 high-quality images</p>
                    <p class="text-xs text-gray-400">First image will be the main display image</p>
                    <p class="text-xs text-gray-400 mt-1">Supported: JPG, PNG, WebP (max 5MB each)</p>
                </div>
            </div>
            ...
            <div id="imagePreviewGrid" class="hidden mt-6">
                ...
                <div class="mt-4 p-4 bg-blue-50 rounded-lg border border-blue-200">
                    <div class="flex items-center text-blue-800">
                        ...
                        <span class="text-sm font-medium">The first image will be used as the main display image on your website</span>
                    </div>
                </div>
            </div>
        ...
        <div class="flex flex-col sm:flex-row gap-4">
            <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-lg font-medium transition-colors">Add Car</button>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function handleFiles(e) {
        console.log('handleFiles called, files:', e.target.files);
        const files = Array.from(e.target.files);
        selectedFiles = files;
        mainImageIndex = 0; // First image is main by default
        console.log('selectedFiles set to:', selectedFiles);
        displayPreviews();
    }
    
    function displayPreviews() {
        console.log('displayPreviews called with', selectedFiles.length, 'files');
        ...
        selectedFiles.forEach((file, index) => {
            console.log(`Processing file ${index}: ${file.name}, isMain: ${index === mainImageIndex}`);
            ...
            reader.onload = function(e) {
                ...
                console.log(`Added preview for file ${index}`);
            };
    ...
    if (form) {
        form.addEventListener('submit', function(e) {
            if (selectedFiles.length < 4) {
                e.preventDefault();
                alert('Please select at least 4 images for the car listing.');
                return false;
            }
